...
b1_n=[abs(umaster(nz1,n1)); abs(umaster(nz1,n2)); abs(umaster(nz1,n3))];
b2_n=[abs(umaster(nz2,n1)); abs(umaster(nz2,n2)); abs(umaster(nz2,n3))];
b3_n=[abs(umaster(nz3,n1)); abs(umaster(nz3,n2)); abs(umaster(nz3,n3))];


for j=1:nloop

    u12=(abs(b1_n)).^2.*b1_n+noise*randn(3,1);
    u14=(abs(b1_n)).^4.*b1_n+noise*randn(3,1);

    u22=(abs(b2_n)).^2.*b2_n+noise*randn(3,1);
    u24=(abs(b2_n)).^4.*b2_n+noise*randn(3,1);

    u32=(abs(b3_n)).^2.*b3_n+noise*randn(3,1);
    u34=(abs(b3_n)).^4.*b3_n+noise*randn(3,1);
...
save  upn_sens3_400 nn2 nn4 k2 k4 j2 j4
